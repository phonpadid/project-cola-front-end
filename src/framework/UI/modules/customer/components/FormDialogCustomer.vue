...
function onSelectLogo(event: FileUploadSelectEvent) {
  ...
  const file = event.files[lastIdx]
  const maxSize = 1024 * 1024

  if (file && file.size > maxSize) {
    invalidMessage.image = 'ຂະໜາດຮູບຕ້ອງບໍ່ເກີນ 1MB'
    return
  } else {
...
async function onSubmit(item: any) {
  ...
  const isSuccess = await props.onSubmit(item, props.form.id as string)
  if (isSuccess) {
    visible.value = false
    clearErrorMessage()
  }
...
<template>
  <Dialog
    ...
    modal
    :header="!props.id ? 'ເພີ່ມລູກຄ້າ' : 'ແກ້ໄຂລູກຄ້າ'"
    ...
    >
      <div>
        <div class="flex flex-column align-items-center justify-content-center">
          <h5 class="flex align-items-center justify-content-center">ຮູບພາບ</h5>
          <FileUpload
          ...
          >
            <template #content>
              <div class="flex flex-wrap p-0 gap-5" v-if="image || props.form.image">
                <div
                ...
                >
                  <div>
                    <img
                      role="presentation"
                      alt="logo image"
                      :src="image ? image.objectURL : props.form.image"
                      class="shadow-2"
                      style="width: 250px"
                    />
                  </div>
      ...
      <div class="grid">
        <div class="col-12 md:col-6">
          <div class="flex flex-column">
            <label for="name" class="font-bold">ຊື່ລູກຄ້າ</label>
            <my-input-text
