...
@singleton()
export class CreateRoleUseCase implements IGUseCase<IResponse<RoleEntity>, CreateRoleDTO> {
  constructor(@inject(RoleApi) private _api: IRoleApi, @inject(Toast) private _toast: IToast) {}

  async execute(input: CreateRoleDTO): Promise<void | IResponse<RoleEntity>> {
    try {
